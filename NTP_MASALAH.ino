...
void writeToCSV(float temperature, float humidity, float voltage, float frequency, unsigned long timestamp) {
    ...
    if (dataFile) {
        // Check if file is not empty and ensure proper formatting
        if (dataFile.size() > 0) {
            dataFile.seek(dataFile.size() - 1);
            char lastChar = 0;
            if (dataFile.available()) {
                lastChar = (char)dataFile.read();
            }
        ...
        verifyCSVFormat();  // Verify after writing
    } else {
...
unsigned long getNextTimestamp() {
    if (wifiConnected && timeClient.update()) {
        // Jika online, gunakan NTP
        lastTimestamp = timeClient.getEpochTime();
    } else {
        // Jika offline, increment 10 detik dari timestamp terakhir
    ...
    if (currentMillis - previousMillisSensor >= SENSOR_INTERVAL) {
        ...
        if (!wifiConnected) {
            ...
            if (currentLines >= MAX_DATA_ROWS) {
                ...
                // Read header and skip oldest data line
