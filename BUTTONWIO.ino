...
// Tambahkan ini
#define BUTTON_1 WIO_KEY_A    // Button 1 pada Wio Terminal
...
// WiFi config
const char* ssid = "IRA-GUEST";
const char* password = "berakhl@k";
bool wifiConnected = false;
unsigned long previousWiFiCheck = 0;
const long WIFI_CHECK_INTERVAL = 14000;

// File and timing
char filename[25] = "/enmostth.csv";
unsigned long previousMillis = 0;
unsigned long previousMillisSensor = 0;
unsigned long lastTimestamp = 0;
const long INTERVAL = 2000;        // Interval kirim data SD
const long SENSOR_INTERVAL = 10000; // Interval baca sensor
...
// FIFO constants
const int MAX_DATA_ROWS = 500000;
...
void writeToCSV(float temperature, float humidity, float voltage, float frequency, unsigned long timestamp) {
    ...
    if (dataFile) {
        ...
        if (fileSize == 0) {
            // New file - write header without newline
            dataFile.print("Temperature;Humidity;Voltage;Frequency;Timestamp");
        }
        
        // Always add newline before new data
        dataFile.printf("\n%.2f;%.2f;%.2f;%.2f;%lu",
...
void loop() {
    ...
    if (digitalRead(BUTTON_1) == LOW) {  // Button ditekan
        if (!screenOn) {
            screenOn = true;
            digitalWrite(LCD_BACKLIGHT, HIGH);
            previousDisplayMillis = currentMillis;  // Reset timer
            Serial.println("Display turned on by button");
  ...
  if (currentMillis - previousMillis >= INTERVAL && wifiConnected) {
    ...
    if (isTransmittingBackup && backupFile) {
        if (backupFile.available()) {
            ...
            // End of file reached - reset file completely
            backupFile.close();
            SD.remove(filename);
            ...
            if (newFile) {
                newFile.print(backupHeader); // Write header without newline
                newFile.close();
            }
            
            isTransmittingBackup = false;
            Serial.println("Backup transmission complete, file reset");
        }
    }
  }
}
